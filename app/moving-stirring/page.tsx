...
const categoryConfig = getCategoryConfig('moving-stirring')
...
export default async function MovingStirringPage() {
  ...
  return (
    <>
      ...
      {/* Breadcrumb Schema */}
      <BreadcrumbSchema items={categoryBreadcrumbs['moving-stirring']} />
      ...
      <main className="min-h-screen bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(450px,1fr))] gap-8">
              {products.map((product) => (
                <ProductCard
                  key={product.id}
                  {...product}
                  ctaPrefix="moving-stirring-category"
                />
              ))}
            </div>
          </section>
          ...
          <section className="text-center py-12 bg-slate-50 rounded-xl">
            ...
            <Link
              href="/contact"
              className="inline-flex items-center px-6 py-3 bg-orange-900 text-white font-semibold rounded-lg hover:bg-orange-800 transition-colors"
            >
              Contact Me
            </Link>
          </section>
