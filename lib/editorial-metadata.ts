...
// Centralized so it can be used by reviews hub, category pages, and anywhere else
...
export const EDITORIAL_METADATA: Record<string, EditorialMetadata> = {
...
// Helper function to get editorial metadata for a product
export function getEditorialMetadata(slug: string): EditorialMetadata | undefined {
  return EDITORIAL_METADATA[slug]
}

// Helper function to get editorial metadata with fallback defaults
export function getEditorialMetadataWithDefaults(slug: string): EditorialMetadata {
  return EDITORIAL_METADATA[slug] || {
    tier: 3,
    testingPeriod: 'Under evaluation',
    hook: 'Professional kitchen equipment evaluation in progress.',
    revenueScore: 0
  }
}
